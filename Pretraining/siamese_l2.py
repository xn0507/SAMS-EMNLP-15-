...
def build_model(tparams, options):
    ...
    emb_x = tparams['Wemb'][x.flatten()].reshape([n_timesteps_x,
                                                n_samples_x,
                                                options['dim_proj']])
    ...
    n_timesteps_xp = xp.shape[0]
    n_samples_xp = xp.shape[1]

    emb_xp = tparams['Wemb'][xp.flatten()].reshape([n_timesteps_xp,
                                              n_samples_xp,
                                              options['dim_proj']])
    ...
    diff = proj_x - proj_xp
    dist = theano.tensor.sqrt((diff ** 2).sum(axis=1) + 1e-8)
    pull = (y * theano.tensor.maximum(0, dist - pull_margin)).sum()
    push = ((1 - y) * theano.tensor.maximum(0, push_margin - dist)).sum()

    cost = pull + c_contrastive * push
...
def pred_error(f_pred, prepare_data, data, iterator, verbose=False):
    ...
    for _, valid_index in iterator:
        ...
        diff = preds_x - preds_xp
        dist = numpy.sqrt((diff ** 2).sum(axis=1) + 1e-8)
        
        pull = (y * numpy.maximum(0, dist - pull_margin)).sum()
        push = ((1 - y) * numpy.maximum(0, push_margin - dist)).sum()
        valid_err += pull + c_contrastive * push
